function updateDataGoogleForm(array, lastRow, ss) {
  for (var i = 0; i < array.length; i++) {
    ...
    if (row.period == 'Факт') {
      var rowPeriod = getParametr(globalVar.parametrArray, 'factPeriod').value
    } else if (row.period == 'Факт-1') {
      var rowPeriod = getParametr(globalVar.parametrArray, 'factPeriod-1').value
    } else if (row.period == 'Бюджет') {
      var rowPeriod = getParametr(globalVar.parametrArray, 'budgetPeriod').value
    } else if (row.period == 'Бюджет+1') {
      var rowPeriod = getParametr(globalVar.parametrArray, 'budgetPeriod+1').value
