function updateDataGoogleForm(array, lastRow, ss) {
  for (var i = 0; i < array.length; i++) {
    ...
    if (row.period == 'Факт') {
      var rowPeriod = getParametr(globalVar, 'factPeriod').value
    } else if (row.period == 'Факт-1') {
      var rowPeriod = getParametr(globalVar, 'factPeriod-1').value
    } else if (row.period == 'Бюджет') {
      var rowPeriod = getParametr(globalVar, 'budgetPeriod').value
    } else if (row.period == 'Бюджет+1') {
      var rowPeriod = getParametr(globalVar, 'budgetPeriod+1').value
